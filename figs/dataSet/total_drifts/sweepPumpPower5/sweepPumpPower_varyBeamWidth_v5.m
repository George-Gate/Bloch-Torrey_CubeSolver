% 计算不同 w/L 下的 bA vs. Imax, dbA/dRrel vs. Imax 曲线
% 只选取三个温度点计算导数
% D, lambda, nRb都随温度变化
% lambda 正比于 L
...
fileName = sprintf('sweepPumpPower_varyBeamWidth_v5 L=%.2fcm %s', cellPars.L(1), datestr(now(),'yyyymmdd_HHMMSS'));
...
for ii = 1:length(w_L_list)
    ...
    for kk = 1:2
        ...
        cP_129.Rrel = cP_129.Rrel*(1+(kk-1.5)*rel_dGamma);    % +-0.5*dGamma      
        cP_131.Rrel = cP_131.Rrel*(1+(kk-1.5)*rel_dGamma);
        
        result_Rrel{ii, kk} = solver.get_pump_power_dependence(I_max_list, pumpBeamProfile, cP_129, cP_131, false);
    end
    fprintf('%d/%d finished. Time elapsed: %.3f hr\n', ii,length(w_L_list), toc(tS)/3600 );
end


save([fileName '.mat'], 'result_Rrel', 'cellPars_129', 'cellPars_131', 'w_L_list', 'rel_dGamma', 'fileName');
...
for ii = 1:2
    tmp = cellfun(@(re)re.R, result_Rrel(:,ii), 'UniformOutput', false);
    R_mat{ii} = cell2mat(tmp');
    bA_mat_Rrel{ii} = -cellPars_129.B0*( abs(R_mat{ii}) - abs(R0))/(R0);

    tmp = cellfun(@(re)re.P_beam_list, result_Rrel(:,ii), 'UniformOutput', false);
    P_beam_mat{ii} = cell2mat(tmp');
    w_L_mat{ii} = repmat(w_L_list,  size(bA_mat_Rrel{ii},1), 1);
end

% Rrel导数
dbA_dRrel_mat = ( bA_mat_Rrel{2} - bA_mat_Rrel{1} ) /rel_dGamma;   % Rrel * dbA/dRrel, in unit of nT
...
hf2 = figure('Position', [190 457 525 426]);
for ii = 1:1
    ax = subplot(1,1,ii);
